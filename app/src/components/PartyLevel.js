...
const PartyLevel = props => {
  ...
  const isComplete =
    status === 'complete' ||
    (departments && departments.length === completeSections);
  ...
  const imgCssClass = isLocked
    ? 'PartyLevel__image--unstarted'
    : 'PartyLevel__image';
  ...
  return (
    <div className={partyLevelCssClass}>
      <img src={`../images/${image}`} alt={title} className={imgCssClass} />
